...
namespace ServitorDiscordBot
{
    public partial class ServitorBot
    {
        private async Task FindSuspiciousAsync(SocketMessage message, bool nigthfalls)
        {
            ...
            var activities = nigthfalls ? await _database.GetSuspiciousNightfallsOnlyAsync(DateTime.Now.AddDays(-7)) : await _database.GetSuspiciousActivitiesWithoutNightfallsAsync(DateTime.Now.AddDays(-7));
        ...
        private async Task RegisterAsync(SocketMessage message, string nickname)
        {
        ...
        private async Task GetUserPartnersAsync(SocketMessage message)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    {
                        var user2 = relation.GetUser2(_database);
